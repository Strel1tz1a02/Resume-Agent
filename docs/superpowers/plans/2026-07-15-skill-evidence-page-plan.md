...
**目标：** 实现独立的技能特长管理页面，让学生可以维护不强制关联经历的 `SkillEvidence` 事实。

**架构：** 前端继续保持轻量，不引入路由库；用当前导航状态在“画像库”和“技能证据”之间切换。复用后端接口：`GET /skill-evidences`、`POST /skill-evidences`、`PUT /skill-evidences/{id}`。`SkillEvidence` 当前只维护 `category` 和 `description`，不保存经历关联。
...
**接口：**
- 消费：浏览器 `fetch`。
- 产出：对技能特长列表、新增、编辑保存行为的前端契约。

- [ ] **步骤 1：写失败测试**

新增测试：渲染 `App` 后点击“技能证据”导航，加载 `/skill-evidences`，列表中展示已有技能特长。断言列表项中描述在分类之前，描述使用黑色正文样式，分类作为标签显示。点击列表项打开编辑弹窗，通过 `PUT /skill-evidences/{id}` 保存 `category` 和 `description`；再点击 `+` 新增技能特长，通过 `POST /skill-evidences` 保存，断言请求体只包含 `category` 和 `description`，不包含 `experience_ids`。

- [ ] **步骤 2：运行测试并确认失败**

运行：`pnpm test`

预期：失败，因为当前应用还没有符合新展示顺序和弹窗编辑契约的技能特长页面。

### 任务 2：技能特长 API 客户端和页面
...
创建技能特长 fetch 客户端，默认使用 `import.meta.env.VITE_API_BASE_URL ?? "http://127.0.0.1:8000"`。类型定义为：
...
新增“技能证据”导航项；主区域在该页显示扁平技能特长列表和加号按钮。列表项上方显示黑色描述，下方显示分类标签。点击列表项打开编辑弹窗，点击加号打开新增弹窗；弹窗只包含分类和技能特长描述两个字段，以及保存、取消操作。
